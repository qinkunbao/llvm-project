...
void CodeGenFunction::EmitParmDecl(const VarDecl &D, ParamValue Arg,
  ...
  RawAddress AllocaPtr = RawAddress::invalid();
  bool DoStore = false;
  ...
  if (Arg.isIndirect()) {
    ...
    if (SrcLangAS != DestLangAS) {
      ...
      auto *T = DeclPtr.getType()->getElementType()->getPointerTo(DestAS);
      DeclPtr = getTargetHooks().performAddrSpaceCast(*this, DeclPtr, SrcLangAS,
                                                   DestLangAS, T, true);
    }
