...
public:
  ...
  Address CreateElementBitCast(Address Addr, llvm::Type *Ty,
    ...
    if (!Addr.hasOffset()) {
      ...
      return Address(CreateBitCast(Addr.getBasePointer(), PtrTy, Name), Ty,
                     Addr.getAlignment(), Addr.getPointerAuthInfo(), nullptr);
    }

    Addr.setElementType(Ty);
    return Addr;
  }
  ...
  Address CreatePointerBitCastOrAddrSpaceCast(Address Addr, llvm::Type *Ty,
    ...
    if (Addr.getType()->getAddressSpace() == Ty->getPointerAddressSpace())
      return CreateElementBitCast(Addr, ElementTy, Name);
  ...
  Address CreateInBoundsGEP(Address Addr, ArrayRef<llvm::Value *> IdxList,
                            ...
                            const Twine &Name = "") {
    if (!Addr.isSigned())
      return {CreateInBoundsGEP(Addr.getElementType(),
                                getRawPointerFromAddress(Addr), IdxList, Name),
              ElementType, Align};
  ...
  Address CreatePreserveStructAccessIndex(Address Addr, unsigned Index,
    ...
    return Address(CreatePreserveStructAccessIndex(ElTy, getRawPointerFromAddress(Addr),
