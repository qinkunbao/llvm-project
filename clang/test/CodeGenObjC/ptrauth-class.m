// RUN: %clang_cc1 -no-opaque-pointers -Wno-everything -fblocks -fptrauth-intrinsics -triple arm64-apple-ios -fobjc-runtime=ios-12.2 -emit-llvm -fobjc-arc -O2 -disable-llvm-passes -o - %s | FileCheck %s
...
// CHECK-LABEL: define void @setTestStructIsa(%struct.TestStruct* noundef %t, i8* noundef %c) #0 {
...
// CHECK-LABEL: define void @setTestClassIsa(%0* noundef %t, i8* noundef %c) #0 {
...
// CHECK-LABEL: define i8* @getTestStructIsa(%struct.TestStruct* noundef %t) #0 {
...
// CHECK-LABEL: define i8* @getTestClassIsa(%0* noundef %t) #0 {
...
// CHECK: define i8* @getTestConstClassIsa([[T_TYPE:%.*]]* noundef %t) #0 {
