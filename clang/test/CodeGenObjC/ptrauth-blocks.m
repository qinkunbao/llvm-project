// RUN: %clang_cc1 -no-opaque-pointers -fptrauth-calls -fptrauth-objc-isa-mode=sign-and-strip -fobjc-arc -fblocks -fobjc-runtime=ios-7 -triple arm64-apple-ios -emit-llvm %s  -o - | FileCheck %s
// RUN: %clang_cc1 -no-opaque-pointers -fptrauth-calls -fptrauth-objc-isa-mode=sign-and-auth  -fobjc-arc -fblocks -fobjc-runtime=ios-7 -triple arm64-apple-ios -emit-llvm %s  -o - | FileCheck %s

void (^blockptr)(void);
// CHECK: @_NSConcreteGlobalBlock.ptrauth = private constant { i8*, i32, i64, i64 } { i8* bitcast (i8** @_NSConcreteGlobalBlock to i8*), i32 2, i64 ptrtoint ({ i8**, i32, i32, i8*, %struct.__block_descriptor* }* @__block_literal_global to i64), i64 27361 }, section "llvm.ptrauth", align 8
...
// CHECK: [[GLOBAL_BLOCK_1]] = internal constant { i8**, i32, i32, i8*, %struct.__block_descriptor* } { i8** getelementptr inbounds ({ i8*, i32, i64, i64 }, { i8*, i32, i64, i64 }* @_NSConcreteGlobalBlock.ptrauth, i32 0, i32 0), i32 1342177280, i32 0, i8* bitcast ({ i8*, i32, i64, i64 }* [[INVOCATION_1]] to i8*),
...
void test_block_literal(int i) {
  ...
  // CHECK:      [[ISAPTRADDR:%.*]] = getelementptr inbounds [[BLOCK_T]], [[BLOCK_T]]* [[BLOCK]], i32 0, i32 0
  // CHECK:      [[ISAPTRADDR_I:%.*]] = ptrtoint i8** [[ISAPTRADDR]] to i64
  // CHECK:      [[ISADISCRIMINATOR:%.*]] = call i64 @llvm.ptrauth.blend(i64 [[ISAPTRADDR_I]], i64 27361)
  // CHECK:      [[SIGNEDISA:%.*]] = call i64 @llvm.ptrauth.sign(i64 ptrtoint (i8** @_NSConcreteStackBlock to i64), i32 2, i64 [[ISADISCRIMINATOR]])
  // CHECK:      [[SIGNEDISAPTR:%.*]] = inttoptr i64 [[SIGNEDISA]] to i8*
  // CHECK:      store i8* [[SIGNEDISAPTR]], i8** [[ISAPTRADDR]], align 8
