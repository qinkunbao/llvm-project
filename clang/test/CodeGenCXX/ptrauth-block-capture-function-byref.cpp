// RUN: %clang_cc1 -no-opaque-pointers -fblocks -triple arm64-apple-ios -fptrauth-calls -emit-llvm -std=c++11 %s -o - | FileCheck %s
...
// CHECK: @global_handler = global void ()* bitcast ({ i8*, i32, i64, i64 }* @_Z8handler2v.ptrauth to void ()*)
...
// CHECK: define void @_Z11callHandlerRFvvE(void ()* noundef nonnull %handler)
