...
// DEFAULT: "-fptrauth-returns" "-fptrauth-intrinsics" "-target-cpu" "apple-a12"{{.*}}
// DEFAULT-KERN: "-fptrauth-returns" "-fptrauth-intrinsics" "-target-cpu" "apple-a12"{{.*}}

// RUN: %clang -target arm64e-apple-ios -fno-ptrauth-returns -c %s -### 2>&1 | FileCheck %s --check-prefix NORET

// NORET-NOT: "-fptrauth-returns"
// NORET: "-fptrauth-intrinsics" "-target-cpu" "apple-a12"
