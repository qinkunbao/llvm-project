// RUN: %clang_cc1 -cc1 -no-opaque-pointers -nostdsysteminc -fptrauth-intrinsics -fptrauth-calls -fptrauth-objc-isa-mode=sign-and-auth -fptrauth-objc-isa-masking -triple arm64-apple-ios -emit-llvm -O0 -disable-llvm-passes -o - %s | FileCheck %s
...
void testWrite(TestStruct1 *t, void *p) {
  t->isa_0_0 = p;
  // CHECK: [[VALUE:%.*]] = ptrtoint i8*
  // CHECK: [[POINTER:%.*]] = and i64 [[VALUE]], ptrtoint (i8* @objc_absolute_packed_isa_class_mask to i64)
  ...
  t->isa_1_0 = p;
  // CHECK: [[VALUE:%.*]] = ptrtoint i8* [[TMP:%.*]] to i64
  // CHECK: [[POINTER:%.*]] = and i64 [[VALUE]], ptrtoint (i8* @objc_absolute_packed_isa_class_mask to i64)
  // CHECK: [[TAG_BITS:%.*]] = xor i64 [[VALUE]], [[POINTER]]
  ...
  t->non_isa = p;
  // CHECK: [[POINTER:%.*]] = ptrtoint i8* [[TMP:%.*]] to i64
  // CHECK: [[SIGNED:%.*]] = call i64 @llvm.ptrauth.sign(i64 [[POINTER]], i32 2
}
