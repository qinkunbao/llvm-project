; RUN: llc -mcpu=apple-a15 -mtriple arm64e-apple-darwin              -asm-verbose=false -o - %s | FileCheck %s
; RUN: llc -mcpu=apple-a15 -mtriple arm64e-apple-darwin -global-isel -asm-verbose=false -o - %s | FileCheck %s
; RUN: llc -mcpu=apple-a15 -mtriple arm64e-apple-darwin -fast-isel   -asm-verbose=false -o - %s | FileCheck %s
...
attributes #0 = { nounwind "ptrauth-returns" "branch-target-enforcement" }
