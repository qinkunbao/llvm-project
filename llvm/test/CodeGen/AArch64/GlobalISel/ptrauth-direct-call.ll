; RUN: llc < %s -mtriple arm64e-apple-darwin -global-isel -verify-machineinstrs -global-isel-abort=1  | FileCheck %s
...
; FIXME: check actual callee value when we can lower ptrauth globals correctly.
; CHECK-LABEL: _test_direct_call_mismatch:
; CHECK:   mov w[[DISC:[0-9]+]], #42
; CHECK:   blrab {{x[0-9]+}}, x[[DISC]]
